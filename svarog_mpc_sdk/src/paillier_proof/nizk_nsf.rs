...
impl NSFProof {
    ...
    pub fn verify(&self, setup: &DLogStatement, N0: &BigInt) -> Outcome<()> {
        let N_tilde = setup.N.clone();
        let s = setup.g.clone();
        let t = setup.ni.clone();
        let sqrt_N0 = N0.sqrt();
        ...
        assert_throw!(z1 > -&bound_alpha_beta && z1 < bound_alpha_beta);
        assert_throw!(z2 > -&bound_alpha_beta && z2 < bound_alpha_beta);
        assert_throw!(N_tilde.bit_length() >= 2046);

        let values = vec![&self.P, &self.Q, &self.A, &self.B, &self.T];
        for value in values {
            assert_throw!(value.gcd(&N_tilde) == BigInt::one());
            assert_throw!(value % &N_tilde != BigInt::zero());
        }
        ...
        assert_throw!(
            (mod_any_pow(&s, &z1, &N_tilde) * mod_any_pow(&t, &w1, &N_tilde)).modulus(&N_tilde)
                == (&self.A * mod_any_pow(&self.P, &e, &N_tilde)).modulus(&N_tilde)
        );
        assert_throw!(
            (mod_any_pow(&s, &z2, &N_tilde) * mod_any_pow(&t, &w2, &N_tilde)).modulus(&N_tilde)
                == (&self.B * mod_any_pow(&self.Q, &e, &N_tilde)).modulus(&N_tilde)
        );
        assert_throw!(
            (mod_any_pow(&self.Q, &z1, &N_tilde) * mod_any_pow(&t, &v, &N_tilde)).modulus(&N_tilde)
